...
const GameConsoleAll = () => {

    const [isAdmin] = useAdmin();
    const getRandomorder = () => Math.random() - 0.5;
    const [products, refetch] = useCategory();

    const geyerdata = products.filter(data => data.categories === 'gameconsole');

    const randomizeddata = geyerdata.sort(getRandomorder);
    ...
    return (
        <div>
            {
                randomizeddata === '' ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4 sm:px-0 px-5'>
                        {
                            randomizeddata.map(w => <div key={w?._id}>
