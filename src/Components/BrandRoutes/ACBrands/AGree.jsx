...
const AGree = () => {
    ...
    const ACGree = products.filter(data => data.categories === 'ac' && data.brands === 'gree');
    ...
    return (
        <div>
            {
                ACGree === '' ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4'>
                        {
                            ACGree.map(w => <div key={w?._id}>
