...
const ASamsung = () => {
    ...
    const ACSamsung = products.filter(data => data.categories === 'ac' && data.brands === 'samsung');
    ...
    return (
        <div>
            {
                ACSamsung === '' ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4'>
                        {
                            ACSamsung.map(w => <div key={w?._id}>
