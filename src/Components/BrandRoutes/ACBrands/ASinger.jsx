...
const ASinger = () => {
    ...
    const ACSinger = products.filter(data => data.category === 'ac' && data.brands === 'acsinger');

    const handleAddtoCart = (singerid) => {

        const { name, image, price, modelname } = products.find(product => product._id === singerid);
