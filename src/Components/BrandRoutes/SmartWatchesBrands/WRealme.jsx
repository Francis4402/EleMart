...
const WRealme = () => {
    ...
    const realmewatch = products.filter(b => b.categories === 'smartwatch' && b.brands === 'realme');
    ...
    return (
        <div>
            {
                realmewatch.length === 0 ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4 sm:px-0 px-5'>
                        {
                            realmewatch.map(w => <div key={w?._id}>
