...
const WApple = () => {
    ...
    const applewatch = products.filter(b => b.categories === 'smartwatch' && b.brands === 'apple');
    ...
    return (
        <div>
            {
                applewatch.length === 0 ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4'>
                        {
                            applewatch.map(w => <div key={w?._id}>
