...
const MMotorola = () => {
    ...
    const motorolaphone = products.filter(b => b.categories === 'mobiles' && b.brands === 'motorola');
    ...
    return (
        <div>
            {
                motorolaphone.length === 0 ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4'>
                        {
                            motorolaphone.map(w => <div key={w?._id}>
