...
const MOnePlus = () => {
    ...
    const oneplusphone = products.filter(b => b.categories === 'mobiles' && b.brands === 'oneplus');
    ...
    return (
        <div>
            {
                oneplusphone.length === 0 ? <h1 className="text-2xl font-semibold min-h-screen w-full justify-center items-center flex">There is no data</h1> :
                    <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-10 my-4 sm:px-0 px-5'>
                        {
                            oneplusphone.map(w => <div key={w?._id}>
